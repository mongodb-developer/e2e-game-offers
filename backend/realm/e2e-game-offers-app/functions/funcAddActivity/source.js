  ...
  let starsAggr = [
    {
      ...
      "$set": {
        "roster.stars": {
          "$switch": {
            "branches": [
              { "case": { "$gte": [ "$roster.shards", 810 ] }, "then": 7 },
              { "case": { "$gte": [ "$roster.shards", 510 ] }, "then": 6 },
              { "case": { "$gte": [ "$roster.shards", 310 ] }, "then": 5 },
              { "case": { "$gte": [ "$roster.shards", 180 ] }, "then": 4 },
              { "case": { "$gte": [ "$roster.shards", 100 ] }, "then": 3 },
              { "case": { "$gte": [ "$roster.shards", 45 ] },  "then": 2 },
              { "case": { "$gte": [ "$roster.shards", 15 ] },  "then": 1 }
            ], 
            "default": 1
          }
        }
